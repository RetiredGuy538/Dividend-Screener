...
def fetch_yfinance(ticker: str) -> dict:
    ...
    try:
        ...
        drop_from_52w = ((high_52w - current_price) / high_52w) * 100 if high_52w else 0
        drop_from_ma50 = ((ma_50 - current_price) / ma_50) * 100 if ma_50 else 0
...
def fetch_fmp(ticker: str, api_key: str) -> dict:
    """Fetch dividend history, payout ratio, historical PE from FMP."""
    ...
    try:
        ...
        div_resp = requests.get(div_url, timeout=10).json()
        dividends = div_resp.get("historical", [])

        # Calculate streak (consecutive years with at least one dividend)
        years_paid = set()
        for d in dividends:
            try:
                yr = int(d["date"][:4])
                if d.get("dividend", 0) > 0:
                    years_paid.add(yr)
            except Exception:
                pass
        ...
        # Key metrics
        metrics_url = f"https://financialmodelingprep.com/api/v3/key-metrics/{ticker}?limit=5&apikey={api_key}"
        metrics_resp = requests.get(metrics_url, timeout=10).json()
        historical_pe_values = [m.get("peRatio") for m in metrics_resp if m.get("peRatio")]
        avg_historical_pe = (sum(historical_pe_values) / len(historical_pe_values)) if historical_pe_values else None

        # Payout ratio
        fin_url = f"https://financialmodelingprep.com/api/v3/ratios/{ticker}?limit=4&apikey={api_key}"
        fin_resp = requests.get(fin_url, timeout=10).json()
        payout_ratios = [r.get("payoutRatio") for r in fin_resp if r.get("payoutRatio")]
        avg_payout = (sum(payout_ratios) / len(payout_ratios)) if payout_ratios else None

        return {
            "dividend_streak": streak,
            "avg_historical_pe": round(avg_historical_pe, 2) if avg_historical_pe else None,
            "avg_payout_ratio": round(avg_payout * 100, 1) if avg_payout else None,
        }
    except Exception as e:
...
def fetch_news_sentiment(sector: str, ticker: str, api_key: str) -> dict:
    ...
    try:
        ...
        negative_words = [
            "crash", "plunge", "fall", "drop", "decline", "sell", "concern",
            "worry", "risk", "loss", "cut", "slump", "fear", "panic", "recession",
            "downgrade", "warning", "threat", "weak", "miss", "disappoint"
        ]
        positive_words = [
            "rise", "gain", "beat", "strong", "growth", "buy", "upgrade",
            "rally", "surge", "record", "profit", "boost", "outperform"
        ]
...
def compute_opportunity_score(data: dict, thresholds: dict) -> dict:
    ...
    # 1. Drop from 52-week high
    drop_52w = data.get("drop_from_52w", 0)
    ticker_threshold = thresholds.get("drop_threshold_pct", 10)
    scores["drop_52w"] = min(100, (drop_52w / ticker_threshold) * 100)

    # 2. Drop from 50-day MA
    drop_ma = data.get("drop_from_ma50", 0)
...
def generate_html(results: list, config: dict, generated_at: str) -> str:
    ...
    for r in results:
        ...
        bar_color = "#ef4444" if score >= 70 else "#f59e0b" if score >= 50 else "#6b7280"
        rows += f"""
        <tr class="data-row" data-ticker="{r['ticker']}" onclick="toggleDetail('{r['ticker']}')">
          ...
          <td class="num">{r.get('dividend_streak', '—')}</td>
          <td class="num {'drop-high' if (r.get('drop_from_52w') or 0) >= 10 else ''}">{r.get('drop_from_52w', '—')}%</td>
          <td class="num">{r.get('drop_from_ma50', '—')}%</td>
            ...
            <div class="detail-panel">
              <div class="detail-grid">
                ...
                <div class="detail-block">
                  ...
                  <p>Dividend Streak: <strong>{r.get('dividend_streak','—')} years</strong></p>
                ...
                <div class="detail-block news-block">
                  ...
                  <p>Sentiment: <strong>{r.get('sentiment_label','—')}</strong></p>
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .tier-strong {{ background: #3f1515; color: #f87171; border: 1px solid #7f1d1d; }}
  ...
  .tier-fair   {{ background: #14291e; color: #4ade80; border: 1px solid #166534; }}
...
<div class="config-bar">
  ...
  <span>Data: <strong>yfinance + FMP + NewsAPI</strong></span>
</div>
...
def main():
    ...
    config = load_config()
    fmp_key = os.environ.get("FMP_API_KEY") or config.get("api_keys", {}).get("fmp", "")
    ...
    for ticker in tickers:
        ...
        fmp_data = fetch_fmp(ticker, fmp_key)
        sentiment_data = fetch_news_sentiment(yf_data.get("sector", ""), ticker, news_key)

        merged = {**yf_data, **fmp_data, **sentiment_data}
